...
def get_geotiff_data_single_label(folder_path, num_geotiffs=100, batch_size=None):
    ...
    # List to store the formatted training data batches and labels
    image_patches = []
    labels = []
        ...
        if filename.endswith(".tif"):
            ...
            try:
                ...
                with rasterio.open(file_path) as dataset:
                    ...
                    if raster_data.shape != (4, 32, 32): # Skip the GeoTiff if it does not have the expected shape
                        continue
            except:
                continue
            ...
            # Extract the label from the 5th channel and create a binary label
            if 'nonforest' in filename:
                label = np.array([0])
            else:
                label = np.array([1])
    ...
    # print('labels\n',labels)
    return convert_to_tf_dataset(input_data, labels)
