...
## QA -- Certain Region to export patches in ############################################
central_african_republic = ee.FeatureCollection("FAO/GAUL/2015/level0").filterMetadata("ADM0_NAME", "equals", "Central African Republic").geometry()
brazil = ee.FeatureCollection("FAO/GAUL/2015/level0").filterMetadata("ADM0_NAME", "equals", "Brazil").geometry()
...
# we pass entire S2 collection to the patch writer func 
s2Coll = (
    s2.filterDate("2018-01-01", "2018-12-31")
    .linkCollection(csPlus, [QA_BAND])
    .map(lambda img: img.updateMask(hexLabel))
    .map(lambda img: img.updateMask(img.select(QA_BAND).gte(CLEAR_THRESHOLD)))
    .map(lambda img: img.addBands(hexLabel))
    .select(["B4", "B3", "B2", "B8", "class"], ["R", "G", "B", "N", "class"])
)
## TESTING ################################################################
def main():
    ...
    ee_points_forest = (
        hexForest
        .filterBounds(brazil)
        .map(lambda h: ee.Feature(h.geometry().centroid()))
    ...
    if args.forest:
        write_geotiff_patch_from_points(
            sampleImage,
            ee_points_forest,
            ["R", "G", "B", "N"],
            10,
